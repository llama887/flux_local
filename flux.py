...
def log_memory_usage(stage):
    ...
    print(f"{stage} - Memory usage: {memory.used / 1e9:.2f} GB (used), {memory.available / 1e9:.2f} GB (available)")
...
text_encoder = CLIPTextModel.from_pretrained(model_id, subfolder="text_encoder", low_cpu_mem_usage=True, device_map="auto")
tokenizer = CLIPTokenizer.from_pretrained(model_id, subfolder="tokenizer", add_prefix_space=False)
text_encoder_2 = T5EncoderModel.from_pretrained(model_id, subfolder="text_encoder_2", low_cpu_mem_usage=True, device_map="auto")
tokenizer_2 = T5TokenizerFast.from_pretrained(model_id, subfolder="tokenizer_2", add_prefix_space=False)
...
pipe.enable_gradient_checkpointing()  # Save memory by recomputing gradients
